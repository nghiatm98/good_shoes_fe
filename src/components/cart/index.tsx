    ...
    <div className="absolute flex flex-col items-center top-full right-0 h-0 overflow-x-hidden transition-all duration-500 rounded-xl group-hover:h-[80vh] w-[400px] group-hover:p-5 bg-white text-black whitespace-nowrap shadow-xl">
      {products?.length ? (
        <>
          <div className="flex flex-col gap-4 w-full">
            <div className="flex flex-row gap-4 items-center">
              ...
              <div className="font-gilroy text-white w-5 h-5 rounded-[50%] bg-black flex justify-center items-center">{products?.length}</div>
            </div>
          ...
          <div className="my-6 flex flex-col gap-5 text-_14 w-full">
            <div className="flex flex-col gap-y-3 ">
              {products.slice(0, 3).map((product) => {
                return <CardItem key={product.id} detail={product} />
              })}
            </div>
          ...
          <div className=" py-6 flex flex-col gap-y-12 w-full">
            ...
            <div className="flex flex-row justify-between font-semibold">
              <span>Tổng</span>
              <span>4,699,000 VNĐ</span>
            </div>
            <div className="flex flex-row gap-4 justify-center">
              <Button label="Thanh toán" className="!h-12 !text-_12" />
            </div>
          </div>
        </>
      ) : (
        <div className="flex flex-col gap-y-4 justify-center items-center h-full">
          ...
          <Button label="Tiếp tục mua hàng" className="!text-_12 !h-12" />
        </div>
      )}
    </div>
  )
